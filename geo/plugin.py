...
_LOGGER = logging.getLogger('action-provider-logger')
...
class PluginMount(type):
    ...
    def __init__(mcs, name, bases, attrs):
        ...
        if not hasattr(mcs, 'plugin_clss'):
            ...
            _LOGGER.debug('Initializing mount point for plugins: %s' % mcs)
            mcs.plugin_clss = []
        else:
            # This must be a plugin implementation, which should be registered.
            # Simply appending it to the list is all that's needed to keep
            # track of it later.
            _LOGGER.debug('New action provider class (plugin): %s' % mcs)
...
class ActionProvider():
    ...
    def __init__(self, title, description, img_path):
